...
describe('User API (e2e)', () => {
  ...
  it('/api/register (POST)', async () => {
    const response = await request(app.getHttpServer())
      .post('/api/register')
      .send({ email: 'tfrte@example.com', password: 'Password123!' }); // This password meets the criteria
  ...
  it('/api/register (POST) with missing fields', async () => {
    const response = await request(app.getHttpServer())
      .post('/api/register') // Corrected endpoint
  ...
  it('/api/register (POST) with invalid email format', async () => {
    const response = await request(app.getHttpServer())
      .post('/api/register')
  ...
  it('/api/register (POST) with weak password', async () => {
    const response = await request(app.getHttpServer())
      .post('/api/register')
  ...
  it('/api/login (POST)', async () => {
    const response = await request(app.getHttpServer())
      .post('/api/login')
      .send({ email: 'tfrte@example.com', password: 'Password123!' });
  ...
  it('/api/users/:id (GET) with invalid token', async () => {
    const response = await request(app.getHttpServer())
      .get('/api/users/1')

...
describe('UserService', () => {
  ...
  it('should register a user and send a welcome email', async () => {
    ...
    jest.spyOn(emailService, 'sendWelcomeEmail').mockResolvedValue(Promise.resolve());
  ...
  it('should throw an error if user is not found during login', async () => {
    ...
    await expect(userService.login('test@example.com', 'password123', {} as Response))
  ...
  it('should throw an error if password is incorrect', async () => {
    ...
    await expect(userService.login('test@example.com', 'wrongpassword', {} as Response))
  ...
  it('should handle errors when Redis fails during logout', async () => {
    jest.spyOn(redisService, 'del').mockRejectedValue(new InternalServerErrorException('Redis error'));
    ...
    await expect(userService.logout('session123', mockRes)).rejects.toThrow(InternalServerErrorException);
  });
});
